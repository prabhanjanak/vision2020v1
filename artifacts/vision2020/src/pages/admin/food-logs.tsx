...
import { useListFoodLogs, getListFoodLogsQueryKey } from "@workspace/api-client-react";
...
export default function FoodLogs() {
  ...
  const { data: logs, isLoading } = useListFoodLogs({
    ...
    query: {
      queryKey: getListFoodLogsQueryKey({ search: debouncedSearch })
    }
  });
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">Review all food collection records</p>
        </div>
                ...
                {isLoading ? (
                  ...
                  logs.map(log => (
                    <TableRow key={log.id}>
                      ...
                      <TableCell className="text-gray-500">{log.coordinatorName || "System"}</TableCell>
                    </TableRow>
                  ))
                ) : (
                  <TableRow>
